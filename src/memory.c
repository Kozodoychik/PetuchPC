...
uint16_t memory_get16(cpu_state* state, uint32_t address)
{
        ...
        return \
		   (uint16_t)region->data[address-region->base] \
		| ((uint16_t)region->data[(address-region->base)+1] << 8);
}
...
uint32_t memory_get32(cpu_state* state, uint32_t address)
{
        ...
        return \
		   (uint32_t)region->data[address-region->base] \
		| ((uint32_t)region->data[(address-region->base)+1] << 8) \
		| ((uint32_t)region->data[(address-region->base)+2] << 16) \
		| ((uint32_t)region->data[(address-region->base)+3] << 24);
}
